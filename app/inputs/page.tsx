...
export default async function InputsPage({ searchParams }: PageProps<"/inputs">) {
  ...
  return (
    <AppShell
      ...
      current="/inputs"
      title="Inputs"
      subtitle="Costs, categories, bills & settings"
    ...
    >
      <div className="mx-auto flex w-full max-w-4xl flex-col">
        ...
        {billsImported !== undefined && (
          <p className="text-sm text-green-700 dark:text-green-400">Imported {billsImported} bill(s).</p>
        )}
        {billErrors.length > 0 && (
          <div className="rounded-2xl border border-amber-400/50 bg-amber-50 px-4 py-3 text-xs text-amber-800 dark:bg-amber-950 dark:text-amber-300">
        ...
        <section className="mt-6">
          ...
          <Card className="mt-3 flex flex-wrap items-end justify-between gap-4 px-5 py-4">
            <form action={saveGeneralSettings} className="flex flex-wrap items-end gap-4">
              <label className="flex flex-col gap-1.5 text-sm text-zinc-600 dark:text-zinc-400">
                ...
                <input
                  ...
                  disabled={readOnly}
                  className="w-32 rounded-lg border border-black/15 px-3 py-2 font-mono text-sm disabled:opacity-50 dark:border-white/20 dark:bg-black"
                />
              ...
              <button
                type="submit"
                disabled={readOnly}
                className="rounded-full bg-teal-600 px-5 py-2 text-sm font-medium text-white disabled:opacity-50"
              >
                Save
              </button>
            </form>
        ...
        <section className="mt-8">
          <CardLabel>Bills &amp; expenses</CardLabel>
          ...
          {bills.length > 0 && (
            <Card className="mt-3 overflow-hidden">
              <ul className="flex flex-col divide-y divide-black/5 dark:divide-white/5">
                {bills.map((bill) => (
                  <li key={bill.id} className="px-5 py-3">
          ...
          <Card className="mt-4 px-5 py-4">
            ...
            <form action={createBillAction} className="mt-3 flex flex-wrap items-end gap-2">
              ...
              <button
                type="submit"
                disabled={readOnly}
                className="rounded-full bg-teal-600 px-4 py-2 text-xs font-medium text-white disabled:opacity-50"
              >
                Add bill
              </button>
            </form>
        ...
        <section className="mt-8">
          <CardLabel>Cost &amp; revenue category per product</CardLabel>
          <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
            ...
            <Card className="mt-3 overflow-hidden">
              <ul className="flex flex-col divide-y divide-black/5 dark:divide-white/5">
                {products.map((product) => {
                  ...
                  return (
                    <li key={product.productId} className="flex flex-wrap items-center justify-between gap-2 px-5 py-3">
                      ...
                      <form action={saveProductCost} className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          placeholder="0.00"
                          disabled={readOnly}
                          className="w-20 rounded-lg border border-black/15 px-2 py-1 font-mono text-sm disabled:opacity-50 dark:border-white/20 dark:bg-black"
                        />
                        <input
                          type="text"
                          ...
                          disabled={readOnly}
                          className="w-36 rounded-lg border border-black/15 px-2 py-1 text-sm disabled:opacity-50 dark:border-white/20 dark:bg-black"
                        />
                        <button
                          type="submit"
                          disabled={readOnly}
...
function BillFields({
  ...
  const inputClass = "rounded-lg border border-black/15 px-2 py-1 text-xs disabled:opacity-50 dark:border-white/20 dark:bg-black";
  return (
    <>
      <input
        type="text"
